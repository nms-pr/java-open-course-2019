        ...
        try{
            ...
            try (FileWriter writer = new FileWriter(args[1],false)){
                for (SyndEntry entry: feed.getEntries()){
                    ...
                    writer.write(entry.getDescription().getValue() == null ? "null" : entry.getDescription().getValue());
                    writer.append(separator);
                    writer.write(entry.getLink() == null ? "null":entry.getLink());
                    writer.append(separator);
                    writer.write(entry.getPublishedDate().toString() == null ? "null" : entry.getPublishedDate().toString());
                    writer.append(separator);
                }
            }
