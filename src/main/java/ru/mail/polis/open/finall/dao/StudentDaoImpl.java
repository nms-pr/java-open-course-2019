...
public class StudentDaoImpl extends DB implements StudentDao {
    ...
    public void saveStudent(Student stud) throws Exception {
        ...
        String sql = "INSERT INTO `student` (`name`, `roll_no`, `phone_no`, `gender`, `email`, "
                + "`address`, `college_name`) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public void updateStudent(Student stud) throws Exception {
        ...
        String sql = "UPDATE `student` SET `name` = ? , `roll_no` = ? ,"
                + " `phone_no` = ? , `gender` = ? , `email` = ?, `address` = ? , `college_name` = ? WHERE `id` = ?";
